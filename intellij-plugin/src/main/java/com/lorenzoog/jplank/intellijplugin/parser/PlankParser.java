...
public class PlankParser implements PsiParser, LightPsiParser {
  ...
  // ( callExpr DOT ) ?
  private static boolean assignExpr_0_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "assignExpr_0_0")) return false;
    assignExpr_0_0_0(b, l + 1);
    return true;
  }

  // callExpr DOT
  private static boolean assignExpr_0_0_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "assignExpr_0_0_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = callExpr(b, l + 1);
  ...
  /* ********************************************************** */
  // IDENTIFIER LBRACE ( argument ( COMMA argument ) * ) ? RBRACE
  public static boolean instanceExpr(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "instanceExpr")) return false;
    if (!nextTokenIs(b, IDENTIFIER)) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeTokens(b, 0, IDENTIFIER, LBRACE);
    r = r && instanceExpr_2(b, l + 1);
    r = r && consumeToken(b, RBRACE);
    exit_section_(b, m, INSTANCE_EXPR, r);
    return r;
  }

  // ( argument ( COMMA argument ) * ) ?
  private static boolean instanceExpr_2(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "instanceExpr_2")) return false;
    instanceExpr_2_0(b, l + 1);
    return true;
  }

  // argument ( COMMA argument ) *
  private static boolean instanceExpr_2_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "instanceExpr_2_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeToken(b, ARGUMENT);
    r = r && instanceExpr_2_0_1(b, l + 1);
    exit_section_(b, m, null, r);
    return r;
  }

  // ( COMMA argument ) *
  private static boolean instanceExpr_2_0_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "instanceExpr_2_0_1")) return false;
    while (true) {
      int c = current_position_(b);
      if (!instanceExpr_2_0_1_0(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "instanceExpr_2_0_1", c)) break;
    }
    return true;
  }

  // COMMA argument
  private static boolean instanceExpr_2_0_1_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "instanceExpr_2_0_1_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
  ...
  public static boolean sizeofExpr(PsiBuilder b, int l) {
    ...
    boolean r;
    Marker m = enter_section_(b);
    r = consumeTokens(b, 0, SIZEOF, IDENTIFIER);
